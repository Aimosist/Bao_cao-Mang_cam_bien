...
app.get('/api/sensor_data/all', (req, res) => {
    const query = `
        SELECT ID, Time, Temp, Hum, Light, MUA
        FROM sensor_data 
        ORDER BY Time ASC;
    `;
...
if (topic === 'outTopic') {
    const regex = /Time: ([\d\- :]+), Temp: ([\d\.]+) °C, Hum: (\d+)%?, Light: (\d+) Lux, MUA: (\d+)/;
    ...
    if (match) {
        ...
        if (mua >= 60) {
            // Nếu AQI > 122, chèn cảnh báo vào bảng devices
            const warningQuery = `INSERT INTO devices (Device, Action, Time) VALUES ('WARNING', 'ON', ?)`;
            db.query(warningQuery, [time], (err, result) => {
                if (err) console.log('Lỗi khi chèn cảnh báo vào devices: ', err);
                else console.log('Đã chèn cảnh báo vào devices thành công: ', result);
            });
        }
        
        if (year >= 2024 && temp > 0 && temp < 50 && humidity > 0 && humidity <= 100 && light > 100 && light <= 1010) {
            const query = 'INSERT INTO sensor_data (Temp, Hum, Light, MUA, Time) VALUES (?, ?, ?, ?, ?)';
            db.query(query, [temp, humidity, light, mua, time], (err, result) => {
